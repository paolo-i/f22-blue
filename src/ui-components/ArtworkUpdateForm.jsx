...
export default function ArtworkUpdateForm(props) {
  const {
    id,
    artwork,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...artworkRecord };
    setArt_name(cleanValues.Art_name);
    setArt_address(cleanValues.Art_address);
    setWhitelist_art(cleanValues.Whitelist_art);
    setUser_ID(cleanValues.User_ID);
    setToken_ID(cleanValues.token_ID);
    setErrors({});
  };
  const [artworkRecord, setArtworkRecord] = React.useState(artwork);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(Artwork, id) : artwork;
      setArtworkRecord(record);
    };
    queryData();
  }, [id, artwork]);
  React.useEffect(resetStateValues, [artworkRecord]);
  const validations = {
    Art_name: [],
    Art_address: [],
    Whitelist_art: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            Artwork.copyOf(artworkRecord, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "ArtworkUpdateForm")}
    >
      <TextField
        label="Art name"
        isRequired={false}
        isReadOnly={false}
        defaultValue={Art_name}
        onChange={(e) => {
          let { value } = e.target;
          if (onChange) {
            const modelFields = {
