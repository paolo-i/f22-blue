...
export default function SettingsUpdateForm(props) {
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...settingsRecord };
    setNotification_Setting(cleanValues.Notification_Setting);
    setProfile_Setting(cleanValues.Profile_Setting);
    setErrors({});
  };
  const [settingsRecord, setSettingsRecord] = React.useState(settings);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(Settings, id) : settings;
  ...
  return (
    ...
    >
      <TextField
        ...
        isRequired={false}
        isReadOnly={false}
        defaultValue={Notification_Setting}
        onChange={(e) => {
          let { value } = e.target;
          if (onChange) {
            const modelFields = {
